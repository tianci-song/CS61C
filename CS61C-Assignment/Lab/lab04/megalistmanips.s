...
mapLoop:
    # 1. mistake: it loads the address of current node rather than the address of
    # the array of the current node
    ...
    # 2. mistake: forget the offset should be 4 bytes long
    ...
    # 3. mistake: forget to save the t registers
    # 3. begin fix
    # before jalr
    addi sp, sp, -4
    sw t1, 0(sp)
    ...
    # 4. mistake: no need to la, just lw is ok
    # la a0, 8(s0)        # load the address of the next node into a0
    # 4. begin fix
    lw a0, 8(s0)
    # 4. end fix
    
    # 5. mistake: s1 itself is the address of the function, no need to 0(s1)
